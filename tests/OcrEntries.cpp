	...
	void setup() {
		std::string arr[] = { " _  _ ",
							  "|_ |_ ",
						  	  " _||_|",
							  "      "};
		OcrEntriesArray entriesArray(arr, arr + 4);
...
TEST(OcrEntries_given_entries_text_of_5_6, then_we_can_retrieve_the_inner_text_later) {
	OcrEntriesArray entriesArray = entries.get()->toArray();
	CHECK(entriesArray[0].compare(" _  _ ") == 0);
	CHECK(entriesArray[1].compare("|_ |_ ") == 0);
	CHECK(entriesArray[2].compare(" _||_|") == 0);
	CHECK(entriesArray[3].compare("      ") == 0);
}
